...
// ------------------------------------------------------------------------------------------------------------
void vAnalysis::step(const cv::Mat &image_rgb, const cv::Mat &image_gray, const cv::Mat &image_lbp,
                     const cv::Mat &movementMask, const cv::Mat &edgeMask) {
...
// bgModelUpdate
// ------------------------------------------------------------------------------------------------------------
void vAnalysis::bgModelUpdate(cv::Mat image_rgb, cv::Mat image_gray, cv::Mat movementMask,
                              cv::Mat edgeMask) {
...
void vAnalysis::draw(cv::Mat image_vis) {
    ...
    for (int jj = m_ROIBbox.y; jj < m_ROIBbox.y + m_ROIBbox.height; jj++) {
        for (int ii = m_ROIBbox.x; ii < m_ROIBbox.x + m_ROIBbox.width; ii++) {
            ...
            // Testy na linie
            for (int k = 0; k <= 3; k++) {
                float e = m_lineParams[k].cos_alfa * ii + m_lineParams[k].sin_alfa * jj -
                          m_lineParams[k].p;

                if (m_lineParams != 0 && m_lineParams[k].gOrL) if (e > 0.0)
                    inside++;
                else if (e <= 0.0)
                    inside++;
            }
            ...
            if (inside == 4) {
                if (jj < 0 || jj >= image_vis.rows || ii < 0 || ii >= image_vis.cols) {
    ...
    cv::putText(image_vis, buf, cv::Point(m_ROIBbox.x, m_ROIBbox.y + 15), 1, 1,
                cv::Scalar(255, 255, 255));
    sprintf(buf, "E = %d|%d", m_edgeCounter, m_edgeThreshold);
    cv::putText(image_vis, buf, cv::Point(m_ROIBbox.x, m_ROIBbox.y + 30), 1, 1,
                cv::Scalar(255, 255, 255));
...
void vAnalysis::computeROIlineParams() {
    ...
    singleLineParam(0, m_analysisROI.TL.x, m_analysisROI.TL.y, m_analysisROI.BL.x,
                    m_analysisROI.BL.y, true);   // TL BL
    singleLineParam(1, m_analysisROI.TR.x, m_analysisROI.TR.y - 5, m_analysisROI.BR.x,
                    m_analysisROI.BR.y - 5, false);  // TR BR
    singleLineParam(2, m_analysisROI.TR.x, m_analysisROI.TR.y, m_analysisROI.TL.x,
                    m_analysisROI.TL.y, true);  // TR TL
    singleLineParam(3, m_analysisROI.BL.x, m_analysisROI.BL.y, m_analysisROI.BR.x,
                    m_analysisROI.BR.y, false);  // BR BL
    ...
    singleLineParam(1, m_analysisROI.TR.x, m_analysisROI.TR.y, m_analysisROI.BR.x,
                    m_analysisROI.BR.y, false);  // TR BR
    // debbug
    //cv::imshow("m_ROIMask", m_ROIMask);
    //cv::waitKey(0);
