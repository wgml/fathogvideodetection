...
int analyseCarPatch_night_zynq(unsigned char rgbImage[][MAX_VDL_WIDTH][4], int vdl_line_width, int car_patch_height) {
	...
	static unsigned char thImage[MAX_VDL_HEIGHT][MAX_VDL_WIDTH];

	for (j = 1; j < car_patch_height; j++) {
		for (i = 1; i < vdl_line_width; i++) {
			if (0.299*(float)(rgbImage[j][i][0]) + 0.587*(float)(rgbImage[j][i][1]) + 0.114*(float)(rgbImage[j][i][2]) > NIGHT_TH)
				thImage[j][i] = 1;
			else
				thImage[j][i] = 0;
