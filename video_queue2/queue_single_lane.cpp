...
queue_single_lane::~queue_single_lane(){
    ...
    //delete m_pvehicleAnalysis;
    //delete[] m_piQueueCounter;
}

void queue_single_lane::initSingleLane(std::string lineName, int nRois, queueROIPolygon* queueROIsPolygon, queueROIPolygon analysisROI, colourRecognition *colourR, typeRecognitionLBP *typeR){
	...
	//Stworzyc tablice obiektow klasy roi
...
void queue_single_lane::step(cv::Mat & image_grey, cv::Mat & image_rgb, cv::Mat & image_lbp, cv::Mat & movementMask, cv::Mat & edgeMask, cv::Mat & darkAreaMask, cv::Mat & image_vis){
	...
	for (int i = 0; i < m_iNqueueROIs; i++) {
		...
		if (i == 0 && !res) {
			...
			if (i == m_iQueueLenght && res) { // analizujemy dane pole
				...
				// prog
				if (m_iQueueCounter[m_iQueueLenght] > 30) { // jesli jest dluzej niz to inkrementujemy dlugosc kolejki
					m_iQueueLenght++;
				}
			}
		}
	...
	cv::rectangle(image_vis, cv::Rect(m_queue_ROIs[0].m_queueROI.BL.x, m_queue_ROIs[0].m_queueROI.BL.y - 35, 75, 25), cv::Scalar(0, 0, 0), CV_FILLED);
	cv::putText(image_vis, buf, cv::Point(m_queue_ROIs[0].m_queueROI.BL.x + 10, m_queue_ROIs[0].m_queueROI.BL.y - 15), 1, 1, cv::Scalar(255, 255, 255));
