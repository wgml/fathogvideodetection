...
sequence::sequence(int width, int height, std::vector<coord> coords, int numLanes, int numRois,
    ...
    for (int lane = 0; lane < numLanes; lane++) {
        ...
        coord lane_tl = std::make_pair(
                tl.first + lane * dXTop + sepLanes / 2,
                tl.second + lane * dYTop + sepLanes / 2
        );
        coord lane_tr = std::make_pair(
                lane_tl.first + dXTop - sepLanes,
                lane_tl.second + dYTop
        );
        coord lane_bl = std::make_pair(
                bl.first + lane * dXBottom + sepLanes / 2,
                bl.second + lane * dYBottom - sepLanes
        );
        coord lane_br = std::make_pair(
                lane_bl.first + dXBottom - sepLanes,
        ...
        for (int roi = 0; roi < numRois; roi++) {
            coord roi_tl = std::make_pair(
                    lane_tl.first + roi * dXLeft + sepRois / 2,
                    lane_tl.second + roi * dYLeft
            );
            coord roi_tr = std::make_pair(
                    lane_tr.first + roi * dXRight - sepRois,
                    lane_tr.second + roi * dYRight
            );
            coord roi_bl = std::make_pair(
                    lane_tl.first + (roi + 1) * dXLeft + sepRois / 2,
                    lane_tl.second + (roi + 1) * dYLeft - sepRois
            );
            coord roi_br = std::make_pair(
                    lane_tr.first + (roi + 1) * dXRight - sepRois,
                    lane_tr.second + (roi + 1) * dYRight - sepRois
            );

            // compatibility thing
            queueROIs[lane][numRois - 1 - roi].TL.x = roi_tl.first;
            queueROIs[lane][numRois - 1 - roi].TL.y = roi_tl.second;
            queueROIs[lane][numRois - 1 - roi].TR.x = roi_tr.first;
            queueROIs[lane][numRois - 1 - roi].TR.y = roi_tr.second;
            queueROIs[lane][numRois - 1 - roi].BL.x = roi_bl.first;
            queueROIs[lane][numRois - 1 - roi].BL.y = roi_bl.second;
            queueROIs[lane][numRois - 1 - roi].BR.x = roi_br.first;
            queueROIs[lane][numRois - 1 - roi].BR.y = roi_br.second;
            queueROIs[lane][numRois - 1 - roi].movementTh = movementTh;
            queueROIs[lane][numRois - 1 - roi].edgeTh = edgeTh;
